        ...
        <ion-content class="pa-4">
            <ion-card class=" pb-3 ma-3" color="dark"  style="border-radius: 25px; padding: 3vh;">
              <div class="d-flex pb-2">
                  ...
                  <ion-label class="px-3 align-self-center" style="font-size: 2.8vh; font-weight: 600;">ImageCapture: {{ user_info.imageCapture_desc }}</ion-label>
                </div>
                <div class="d-flex pb-2">
                  <ion-icon :icon="location" size="large" style="scale: 1.2 !important; width:65px;" class="pa-2"/>
                  <ion-label class="px-2 align-self-center" style="font-size: 2.8vh; font-weight: 600;">GeoFencing: {{ user_info.geoFenceMode_desc }}</ion-label>
                </div>
                ...
                <h4 class="d-flex justify-center pt-1" style="font-size: 2.5vh;">Max Location: {{ user_info.noOfLocations }}</h4>
              </ion-card>
            <h4 class="px-4" style="font-size: 2.8vh;">Configured Locations({{ Locations.length }}):</h4>
            <ion-list  class="mx-3" style="height: 40vh; overflow-y: auto;">
              <ion-item v-for="(loc, key) in Locations" :key="key"> 
                <div class="d-block pt-2">
                  <ion-label style="font-size: 2.3vh;">{{ address[key] ? address[key] : 'Searching Address' }}</ion-label>
                  <div class="d-flex ">
                    <h5 class="pe-5 mb-1" style="font-size: 1.8vh;">Lat: {{ loc.lat }}</h5>
                    <h5 style="font-size: 1.8vh;">Long: {{ loc.long }}</h5>
                  </div>
...
export default {
  ...
  async created(){
    ...
    try{
      ...
      this.showAlert({header: 'Warning', message: 'An error occur while fetching address', buttons: ['Okay'], })
    }
